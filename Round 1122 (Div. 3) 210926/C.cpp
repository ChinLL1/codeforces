...
int main() {
  ...
  while (t--) {
    ...
    // if s[0] is 1 then we need everyone to become 1
    
    LL oneCount = 0, zeroCount = 0;
    for (auto& inner : s) {
        if (inner == '0') zeroCount++;
    }
    if (s[0] == '1') cout << zeroCount << endl;
    else {
        LL zerosRight = zeroCount, onesLeft = 0;
        LL res = zerosRight; // k = 0 split (s[0] is 0, so this counts it; see below)
        for (int i = 0; i < n; i++) {
            if (s[i] == '1') onesLeft++;
            else zerosRight--;
