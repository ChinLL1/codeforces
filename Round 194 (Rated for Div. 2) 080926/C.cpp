...
int main() {
  ...
  while (t--) {
    ...
    cin >> x >> y;
    LL answer = x + y;
    // we just have to find the largest value that is smaller than x and is a submask of answer

    bitset<30> A(answer), X(x);
    bitset<30> biggestVal(0);
    bool alreadySmaller = false;
    for (int i = 29; i >= 0; i--) {
      if (A.test(i)) {
        if (alreadySmaller) {
          biggestVal.set(i);
        ...
        if (X.test(i)) alreadySmaller = true;
      }
    }
    cout << answer << " " << x - biggestVal.to_ullong() << endl;
  }    
}