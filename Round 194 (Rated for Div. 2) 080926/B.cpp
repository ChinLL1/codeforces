...
int main() {
  ...
  while (t--) {
    LL employees, projects, months;
    cin >> employees;
    cin >> projects; 
    cin >> months;
    LL res = 0;
    LL diff = projects - employees;
    if (projects < employees) {
        cout << months * projects + months * (months - 1) / 2 << '\n';
        continue;
    }
    LL copyOfEmployees = employees;
    LL split = min(employees + diff, employees + months);
    for (LL i = employees; i < split; ++i) {
      res += diff % employees;
      employees++;
    }
    LL remaining = months - (split - copyOfEmployees);
    if (remaining > 0) res += remaining * diff;
    cout << res << endl;
  }    
}