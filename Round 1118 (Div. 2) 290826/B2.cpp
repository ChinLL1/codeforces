...
int main() {
    ...
    while (t--) {
        ...
        vector<LL> suffix(m + 2, 0);
        for (int v = m; v >= 1; v--) {
            suffix[v] = suffix[v + 1] + cnt[v];
        }

        for (int k = 1; k <= m; k++) {
            LL res = 0;
            for (int T = 1; T <= m; T++) {
                LL total = suffix[T]; // cnt[T] + everything larger (base case)

                // walk the doubling chain T, 2T, 4T, ... up to depth k
                long long mult = 2;
                for (int depth = 1; depth <= k; depth++) {
                    long long level = (long long)T * mult;
                    if (level > m) break;
                    total += cnt[level];
                    mult *= 2;
                }
